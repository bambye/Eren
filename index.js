...
const {wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, información, success, clos } = require('./lib/functions');
...
async function starts() {
	...
	fs.existsSync('./Lexa.json') && Lxa.loadAuthinformación('./Lexa.json')
	Lxa.on('connecting', () => {
		start('2', 'Connecting...')
	})
	Lxa.on('open', () => {
	...
	await Lxa.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Lexa.json', JSON.stringify(Lxa.base64EncodedAuthinformación(), null, '\t'))
...
Usa ${prefix}verify para verificarte y poder usar el bot.`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextinformación: {"mentionedJid": [num]}})
...
//--Balasan bot
			...
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? Lxa.sendMessage(from, teks.trim(), extendedText, {contextinformación: {"mentionedJid": memberr}}) : Lxa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextinformación: {"mentionedJid": memberr}})
			}
...
*/
	...
	break
					case 'slow':
					low = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextinformación
				...
				break
				case 'tupai':
					pai = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextinformación
				...
				break
				case 'gemuk':
					muk = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextinformación
				...
				break
				case 'bass':                 
					ass = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextinformación
...
caption: teks, contextinformación: {
  mentionedJid: [me.jid]}})
  break
...
case 'listonline':
  			  ...
  			  contextinformación: { mentionedJid: online }
			    })
				break
...
const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextinformación: mek
reply(mess.wait)
...
case 'triggered':
 ...
 if (!isRegister) return reply(mess.only.userB)
           ...
           ger = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextinformación : mek
           reply(mess.wait)
...
var imgbb = require('imgbb-uploader')
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
  ted = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextinformación: mek
  reply(mess.wait)
...
var gh = body.slice(7)
mentioned = mek.message.extendedTextMessage.contextinformación.mentionedJid
...
reply(teks.trim())
if (data.message) return reply(`La informaciónrmación de  *${teks} no se encontro`)
...
//--información gempa
//informaciónrmasi gempa terkini
case 'informacióngempa':
  if (!isRegister) return reply(mess.only.daftarB)
  
  Lxa.updatePresence(from, Presence.composing)
  try {
  anu = await fetchJson(`https://docs-jojo.herokuapp.com/api/informacióngempa`, {
method: 'get'
  })
...
method: 'get'
  ...
  if (anu.error) return reply(anu.error)
  teks = `*Nombre* : ${anu.result.nameinformación}\n*Caption* : ${anu.result.textinformación}\n\n_Enviando archivos, espere_`
...
mimetype: 'video/mp4', filename: `${anu.nameinformación}.mp4`, quoted: mek
  })
...
//--block user
				case 'blocklist':
					...
					Lxa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextinformación: {"mentionedJid": blocked}})
					break
				  ...
				  case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.toFormat('webp')
							.save(ran)
					} else if ((isMedia && mek.message.videoMessage.seconds < 11 || isQuotedVideo && mek.message.extendedTextMessage.contextinformación.quotedMessage.videoMessage.seconds < 11) && args.length == 0) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextinformación : mek
						const media = await Lxa.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
						reply(mess.wait)
						await ffmpeg(`./${media}`)
...
//-- temp
			...
			case 'liststik':
				...
				Lxa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextinformación: { "mentionedJid": setiker } })
				break
			...
			case 'totaluser':
				...
				Lxa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextinformación: { "mentionedJid": _registered} })
				break
			...
			case 'addstik':
				...
				boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextinformación
			...
			case 'vnlist':
				...
				Lxa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextinformación: { "mentionedJid": audionye } })
				break
			...
			case 'listimg':
				...
				Lxa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextinformación: { "mentionedJid": imagenye } })
				break
			...
			case 'listvid':
				...
				Lxa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextinformación: { "mentionedJid": videonye } })
				break
...
reply(mess.wait)
anumedia = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextinformación
...
reply(mess.wait)
mitri = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextinformación
...
//--Hilih maker
				...
				case 'tiktokstalk':
					try {
						...
						let { user, stats } = await tiktod.getUserProfileinformación(args[0])
						reply(mess.wait)
...
case 'setppbot':
  Lxa.updatePresence(from, Presence.composing)
  if (!isOwner) return reply(mess.only.ownerB)
  const botpp = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contxtinformación: mek
...
//--Convert stiker to image
				case 'toimg':
					...
					if (!isRegister) return reply(mess.only.daftarB)
					reply(mess.wait)
					imgmed = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextinformación
...
Lxa.deleteMessage(from, {
  id: mek.message.extendedTextMessage.contextinformación.stanzaId, remoteJid: from, fromMe: true
})
} catch {
...
break
				...
				case 'clone':
					...
					mentioned = mek.message.extendedTextMessage.contextinformación.mentionedJid[0]
